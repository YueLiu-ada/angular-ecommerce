...
export class CartService {
  ...
  // subject is a subclass of observable. we can use subject to push events in our code
  ...
  addToCart(theCartItem: CartItem){
    ...
    if(this.cartItems.length > 0){
      ...
      // find method will return first element that passes else returns undefined
  ...
  computeCartTotals() {
    ...
    // next() method is used to publish and send event
  ...
  remove(theCartItem: CartItem) {
    // get idx of item in the array
    const itemIndex = this.cartItems.findIndex( tempCartItem => tempCartItem.id === theCartItem.id);
    // if found, remove the item from the array at the given index
    if(itemIndex > -1){ // find it
